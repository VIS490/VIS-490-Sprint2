...
class ScoresTest(unittest.TestCase):
    ...
    def test_split_data_failure(self):
        ...
        for test in self.failure_split_data_test_params:
            score_test = score.ScoresGenerator()
            score_test.split_data(test[KEY_INPUT])
            self.assertNotEqual(score_test.SCORES[KEY_WORK_LOAD],
            test[KEY_EXPECTED][KEY_WORK_LOAD])
            self.assertNotEqual(score_test.SCORES[KEY_INDEPENDENCE],
            test[KEY_EXPECTED][KEY_INDEPENDENCE])
            self.assertNotEqual(score_test.SCORES[KEY_LEADER_SUPPORT],
            test[KEY_EXPECTED][KEY_LEADER_SUPPORT])
            self.assertNotEqual(score_test.SCORES[KEY_PEER_RELATIONSHIPS],
            test[KEY_EXPECTED][KEY_PEER_RELATIONSHIPS])
            self.assertNotEqual(score_test.SCORES[KEY_CONTRIBUTION_IMPACT],
            test[KEY_EXPECTED][KEY_CONTRIBUTION_IMPACT])
            self.assertNotEqual(score_test.SCORES[KEY_DEVELOPMENT],
            test[KEY_EXPECTED][KEY_DEVELOPMENT])

    def test_create_category_scaled_score_success(self):
        """Score Test scaled score"""
        score_test = score.ScoresGenerator()
